...
Eigen::Matrix4f Recognizer::aligner(pcl::CorrespondencesPtr corrs, pcl::PointCloud<PointType>::Ptr source_keypoints, pcl::PointCloud<PointType>::Ptr target_keypoints, float cg_thresh) {
	...
	for(float cg_size = 0.01f; cg_size < 0.03; cg_size += 0.005f) {
		...
		for(size_t i = 0; i < rototranslations.size(); ++i) {
			...
			//pcl::visualization::PointCloudColorHandlerCustom<PointType> rotated_source_color_handler_gc(rotated_source, 0, 255, 0);
			//viewer->addPointCloud(rotated_source, rotated_source_color_handler_gc, "rotated_source");
			viewer->addPointCloud(target_keypoints, std::string("target"));
			...
			if(score < f_min) {
				...
				pcl::visualization::PointCloudColorHandlerCustom<PointType> registered_source_color_handler(registered_source, 255, 0, 0);
				pcl::transformPointCloud(*source_keypoints, *registered_source, best_transform);
				viewer->addPointCloud(registered_source, registered_source_color_handler, "registered_source");
				viewer->spinOnce(2000);
				viewer->removePointCloud("registered_source");
			}
			
			viewer->removePointCloud("target");
			//viewer->removePointCloud("rotated_source");
		}
	}
